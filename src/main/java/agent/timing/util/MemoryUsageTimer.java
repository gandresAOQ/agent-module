...
public class MemoryUsageTimer {
    ...
    public static void end(String threadName, long usedMemoryBefore, String className, String method) {
        ...
        EXECUTOR.submit(() -> MemoryUsageTimer.logMemoryUsage(className, method, Double.valueOf(usedMemoryAfter)));
    }

    private static void logMemoryUsage(String className, String methodName, Double value) {
        try {
            Map<String, String> data = new HashMap<>();
            data.put("methodName", methodName);
            data.put("className", className);
            data.put("fullName", className + "." + methodName);
            MongoReporter.report(data, "memory", value);
        } catch (Exception e) {
